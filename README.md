...
- **Report a hazard** — a floating **Report Hazard** button opens a dialog with an optional message, reporter name, incident time, and **photo** (JPEG/PNG/WebP, ≤5 MB), plus a required location picker (tap the map or **use my location**). Submitting sends a `multipart/form-data` `POST /api/reports` (`latitude`, `longitude`, and any provided optional fields/image) and immediately closes the dialog with a success toast (fire-and-forget, no waiting on the network).
